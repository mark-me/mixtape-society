    ...
    def browse() -> Response:
        ...
        if has_multiple_collections:
            ...
            for mixtape in mixtapes:
                ...
                mixtape["collection_id"] = (
                    collection_id or collection_manager._default_id
                )
        ...
        if collection_filter:
            mixtapes = [
                m for m in mixtapes if m.get("collection_id") == collection_filter
            ]
        ...
        if sort_by == "title":
            mixtapes.sort(
                key=lambda x: x.get("title", "").lower(), reverse=(sort_order == "desc")
            )
        elif sort_by == "created_at":
            mixtapes.sort(
                key=lambda x: x.get("created_at") or "", reverse=(sort_order == "desc")
            )
            ...
            mixtapes.sort(
                key=lambda x: len(x.get("tracks", [])), reverse=(sort_order == "desc")
            )
